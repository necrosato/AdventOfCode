...
def get_neighbors(grid, row, col):
    ...
    for offi, offj, allowed in [(-1, 0, '^'), (1, 0, 'v'), (0, -1, '<'), (0, 1, '>')]:
        i = row+offi
        j = col+offj
        if 0 <= i < rows and 0 <= j < cols and grid[i][j] in ['.',allowed]:
...
def dfs_grid(grid, source, dest, visited=None, path=None, paths=None):
    row, col, val = source
    if visited==None:
        visited=set()
    if path==None:
...
'''
    ...
    for neighbor in get_neighbors(grid, row, col):
        if neighbor not in visited:
            dfs_grid(grid, neighbor, dest, visited, path, paths)
