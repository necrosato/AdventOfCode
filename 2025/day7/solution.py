...
def part1( grid ):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[-1])):
            if grid[i][j] == 'S':
                beams.add((i,j))
            if grid[i][j] == '.' and (i-1,j) in beams:
                beams.add((i,j))
            if grid[i][j] == '^' and (i-1,j) in beams:
                splits += 1
                beams.add((i,j-1))
...
def part2( grid ):
    ...
    memo = {}
    def paths_to_bottom(grid,i,j):
        if i not in range(len(grid)) or j not in range(len(grid[0])):
            return 1
        if (i,j) in memo:
            return memo[(i,j)]
        elif (i,j) in beams:
            paths = paths_to_bottom(grid,i+1,j)
        elif grid[i][j] == '^' and (i-1,j) in beams:
            paths = paths_to_bottom(grid,i,j+1) + paths_to_bottom(grid,i,j-1)
        memo[(i,j)] = paths
        return paths
    return paths_to_bottom(grid,start[0],start[1])
